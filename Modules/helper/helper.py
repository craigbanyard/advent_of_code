...
from typing import Any, Callable, Iterator, ItemsView
...
class Colours:
    ...
    def __repr__(self) -> str:
        ...
        def attrs_to_str(cls, label) -> str:
            """
            Return a string representation of the public attributes of cls with type
            str.
            """
            out = f"{label}\n"
            attrs = {
...
class Grid:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        Keyword arguments:
          ...
          diag: Indicator of whether diagonal moves are valid
        """
        self.G = kwargs.get("G", [[]])
        self._R = len(self.G)
        self._C = len(self.G[0])
        self.grid_dict = {
            (r, c): self.G[r][c] for c in range(self._C) for r in range(self._R)
        }
    ...
    def _init_cost_dict(self, start_nodes: list[Node]) -> CostDict:
        """Initialise cost dictionary for grid traversal."""
        return defaultdict(lambda: math.inf, {s: 0 for s in start_nodes})

    def _traversal_complete(
        self, node: Node, cost: CostDict, end: Any, max_cost: int | float
    ) -> bool:
    ...
    def cost(self, node: Node) -> int:
        """Cost to move to node."""
        return 1

    def h(self, a: Node, b: Node) -> int:
        """
        A* heuristic for moving between nodes a and b. Default implementation is
        Manhattan distance.
        """
        return sum(abs(p - q) for p, q in zip(a, b))

    def bfs(
        self, start: Any, end: Any, max_cost: int | float = math.inf
    ) -> tuple[VisDict, CostDict]:
    ...
    def run_algorithm(
        self, start: Any, end: Any, algorithm: str
    ) -> tuple[VisDict, CostDict]:
    ...
    def shortest_path(self, start: Any, end: Any, algorithm: str) -> int | float:
        """
    ...
    def construct_path(self, start: Any, end: Any, visited: VisDict) -> set[Node]:
        """
        Return a set of nodes (r, c) that comprise the path from start to end implied
        by the visited dictionary. If end is None, return the set of visited nodes.
        """
        if end is None:
    ...
    def optimal_path(self, start: Any, end: Any, algorithm: str) -> set[Node]:
        """
        Return a set of nodes (r, c) that comprise the optimal path from start to end
        determined by performing the specified search algorithm. If end is None, return
        the set of visited nodes.
        """
        visited, _ = self.run_algorithm(start, end, algorithm)
        return self.construct_path(start, end, visited)

    def visualise_path(self, path: set[Node], **kwargs) -> str | None:
        """
        Return a string representation of the path provided as the argument. Optimal
        path marker can be provided as the marker keyword argument.
        """
        if not path:
            return None
...
def aoc_timer(
    ...
    *,
    repeat: int = 1,
    metric: Callable | str = min,
    **margs
):
    """
    Decorator that times the call of a function, func, and prints its execution time. If
    If func is called with keyword argument time=False, the print will be suppressed.

    With optional repeat, run func repeat times, appending successive execution times to
    list (or numpy array) [times].
    ...
    - Expected numpy: https://numpy.org/doc/stable/reference/routines.statistics.html
    ...
    def mean_std(times: ArrayLike) -> str:
        m, s = np.mean(times), np.std(times)
        return f"{format_time(m)} ± {format_time(s)}"

    def err(times: ArrayLike, metric: Callable | str = metric) -> str:
