...
def straight(pwd):
    return sum(ord(z) == ord(y)+1 == ord(x)+2 for x,y,z in zip(pwd,pwd[1:],pwd[2:])) > 0

def two_pair(pwd):
    groups = [len([1 for _ in g]) for k,g in groupby(pwd)]
    return sum(1 for i in groups if i > 1) > 1

# # Just check conditions, brute force
# def Day11(pwd):
#     while True:
#         pwd = inc_pwd(pwd)
#         if eight_lower(pwd):
...
# Replace illegal characters, don't check eight lower (never bites in this problem) - saves iterations
def Day11(pwd):
    while True:
